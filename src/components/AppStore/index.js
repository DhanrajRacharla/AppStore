...
class AppStore extends Component {
  state = {
    ...
    appsList: appsList,
    initialAppsListCategory: appsList[0].category,
    initialTabsListTabId: tabsList[0].tabId,
    searchInput: '',
  }

  switchToNextTab = tabId => {
    this.setState({initialTabsListTabId: tabId, initialAppsListCategory: tabId})
  }

  searchTheItem = event => {
    const updateList = appsList.filter(eachList =>
      eachList.appName.toLowerCase().includes(event.target.value.toLowerCase()),
    )
    this.setState({searchInput: event.target.value, appsList:updateList})
  }
  ...
  render() {
    const {
      initialTabsList,
      appsList,
      initialAppsListCategory,
      initialTabsListTabId,
      searchInput,
    } = this.state
      ...
      <div className="bg-container">
        <div className="first-container">
          ...
          <ul className="unordered-list-1">
            {initialTabsList.map(eachTab => (
              <TabItem
                key={eachTab.tabId}
                eachTab={eachTab}
                initialTabsListTabId={initialTabsListTabId}
                switchToNextTab={this.switchToNextTab}
              />
            ))}
          </ul>
        </div>
        ...
        <div className="second-container">
          <ul className="unordered-list-2">
            {appsList.map(eachList => (
              <AppItem
                key={eachList.appId}
                eachList={eachList}
                initialAppsListCategory={initialAppsListCategory}
              />
            ))}
          </ul>
        </div>
